...
function addSeries(left: Array<number | null> | null | undefined, right: Array<number | null> | null | undefined, length: number): Array<number | null> {
  const a = left ?? zeroSeries(length);
  const b = right ?? zeroSeries(length);
...
function grossRevenueByMetal(
  qtyByMetal: Record<string, Array<number | null>>,
  priceByMetal: Record<string, Array<number | null>>,
  masterN: number,
...
function payabilityDeductions(
  grossByMetalUSD: Record<string, Array<number | null>>,
...
function buildFiscalLedger(args: {
  ...
  const supplied = input.phase1.fiscalLedgerUSD ?? {};
  const op = input.phase1.operatingCostsUSD;
  const ga = input.phase1.siteGandA_USD;
  const sustaining = input.phase1.sustainingCapexUSD;
  const reclamation = input.phase1.reclamationUSD;
  const dep = input.phase1.depreciationUSD ?? zeroSeries(length);
  const bp = input.phase1.byproductCreditsUSD ?? zeroSeries(length);
  ...
  const ebitdaBeforeFiscal = Array.from({ length }, (_, t) => {
    const values = [revenueAfterStreamUSD[t], op[t], baseSellingCostsUSD[t], ga[t], reclamation[t], bp[t]];
    if (!values.every(finite)) return null;
    return (values[0] as number) - (values[1] as number) - (values[2] as number) - (values[3] as number) - (values[4] as number) + (values[5] as number);
  });
...
export function computeProjectEngineFullProductionV1(input: ProjectEngineFullProductionV1Input): ProjectEngineFullProductionV1Output {
  ...
  const streamsOut = hasStreams ? applyStreamsByMetal({
  ...
  const grossPreStreamByMetal = grossRevenueByMetal(input.payableQtyByMetal, input.spotPriceUSDByMetal, input.masterN);
