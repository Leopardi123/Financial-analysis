...
(function runEngineFullProductionV1Tests() {
  ...
  const override = computeProjectEngineFullProductionV1({
    masterN: 2,
    streamsByMetal: null,
    payableQtyByMetal: { Au: [1, 1, 1] },
    spotPriceUSDByMetal: { Au: [0, 1000, 0] },
    takeItems: [{
      id: 'nsr', jurisdictionLevel: 'national', metals: ['ALL'],
      baseType: 'REVENUE', rateType: 'FIXED', rateFixed: 0.05,
    }],
    phase1: {
      ...
      royaltiesUSD: [0, 999, 0],
    },
    phase2: { discountRate: 0.1 },
    aisc: { auPriceUSDPerOz: [1000, 1000, 1000] },
  });

  if (override.totalTakeUSD[1] !== 50) throw new Error('derived take expected at t=1');
  if (override.phase1.sustainingCostUSD[1] !== 999) throw new Error('royalties override should feed phase1');
  if (override.phase1.ebitUSD[1] !== 1) throw new Error('override should affect EBIT');
  if (override.phase1.fcffUSD[1] !== 1) throw new Error('override should affect FCFF');
