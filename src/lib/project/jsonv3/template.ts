...
export function buildProjectJsonV3Template(): ProjectJsonV3 & Record<string, unknown> {
  ...
  return {
    ...
    _how_to_fill: [
      ...
      '4. time.runtimePlacement is the movable calendar anchor. Set productionStartYear only from the latest verified company schedule/guidance. If guidance changes, update runtimePlacement only; the relative economic arrays must remain unchanged unless the mine plan itself changes.',
      '5. reportPeriodLabels are report evidence such as -3,-2,-1,1,2... or Year 1, Year 2. They are not calendar years and may be null if the report does not disclose labels. Do not invent labels.',
      '6. Use null for unknown/unverified report data. Use 0 only when the report explicitly shows zero or the item is verified not applicable. Never replace missing information with zero just to make the model run.',
      '7. Add only metals that participate in the report economic model. Do not add a metal merely because it is common for the project type.',
      '8. payableQtyByMetal must represent payable quantities. Do not silently substitute mined, contained or recovered metal when the report distinguishes those concepts.',
      '9. priceKeyByMetal values are runtime API-series identifiers. Never guess a price key. Verify the exact supported key before entering it.',
      '10. costModel has exactly one active source. Use AGGREGATE when the report only supports aggregate site operating cost. Use COMPONENTS when mining/processing/site G&A or other site OPEX can be mapped without overlap. Never provide both.',
      '11. sellingModel has exactly one active source. Put TC, RC, freight, insurance, marketing and other off-site deductions here when separately disclosed. Do not also hide the same amount inside site OPEX.',
      '12. royaltyModel has exactly one active source. Use RULES when the royalty/stream formula can be represented faithfully; use LOCKED_SERIES only when the report supplies a period cash-flow series that cannot be reconstructed; use NONE only when no project royalty/take applies.',
      '13. taxModel has exactly one active source. FLAT_RATE is allowed only when that is a faithful model of the report/runtime tax assumption. LOCKED_SERIES is report-deck locked and is not automatically suitable for spot sensitivity. UNKNOWN must be resolved before runtime.',
      '14. By-product revenue belongs in metal revenue once. Do not add a parallel by-product-credit project income series. Net-by-product treatment is a derived C1/AISC recipe, not a second revenue source.',
      '15. capexUSD, sustainingCapexUSD, closureUSD, workingCapitalDeltaUSD and terminalProceedsUSD must be placed in the same RELATIVE periods as the report. Do not move terminal/closure/WC items to make NPV match.',
      '16. If the report provides only a LOM average and no defensible annual schedule, do not fabricate an annual series by interpolation. Use the supported aggregate representation only if it truthfully preserves the disclosed economics; otherwise leave the item unverified.',
      '17. operations is evidence/physical detail used by Project and Tier. Populate only what the report supports. Units must be explicit and consistent with the source.',
      '18. verification.report is a report checkpoint contract, not a second cash-flow model. Store report price deck, discount rate/convention, report NPV/IRR and hard-check totals/source pages. Do not store parallel report FCFF arrays.',
      '19. The report price deck in verification.report.priceDeckByKey must use the same verified runtime price keys as metals.priceKeyByMetal. No implicit FX or substitute prices are allowed in reconciliation.',
      '20. Record the source/table/page for NPV/IRR, prices, report periods and major assumptions. Payability, TC/RC, royalties, tax and other material assumptions must match the technical report economic case used for report NPV/IRR.',
      '21. reportedCostCheckpoints contains report C1/AISC checkpoints only. These values must never override Project/Corporate economics; they are used to test reconstructed cost metrics and benchmark compatibility.',
      '22. Schema-valid/runnable is not the same as VERIFIED. A project is VERIFIED only after the SAME Project engine, run with the report deck on the same relative period series, reproduces report NPV and IRR within tolerance and all mandatory period/CAPEX/closure/WC/assumption hard checks pass.'
    ],
    _single_source_rules: {
      timeline: 'Relative economic period index t=0..masterN is canonical. runtimePlacement only maps productionStartPeriod to the current expected calendar year; it must not create a second economic timeline.',
    ...
    _calendar_placement_rule: {
      ...
      runtime_anchor: 'time.runtimePlacement.productionStartYear maps productionStartPeriod to the latest company-guided production-start calendar year.',
      update_rule: 'A schedule delay/acceleration changes runtimePlacement only. Shift economic arrays only if the underlying technical mine plan changes.',
    ...
    _mapping_examples: {
      schedule_change: 'PFS economics have productionStartPeriod=3. If guidance moves production start from 2031 to 2033, change runtimePlacement.productionStartYear from 2031 to 2033. Do not shift any economic array.',
