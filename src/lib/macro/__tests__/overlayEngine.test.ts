...
{
  const rows = buildRows();
  // Simulate runtime where canonical ACMTP10 key exists but carries null values,
  // while numeric repricing observations are present on lu_repricing_us.
  for (let i = 0; i < 24; i += 1) {
    const month = String((i % 12) + 1).padStart(2, "0");
    const year = 2023 + Math.floor(i / 12);
    rows.push({ series_key: "ACMTP10", date: `${year}-${month}-28`, value: null });
  }
