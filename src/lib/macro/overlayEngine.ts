...
export function buildSeriesMap(rows: Array<{ series_key: string; date: string; value: number | null }>): SeriesMap {
  ...
  const finiteCount = (points: Point[]): number => points.filter((point) => typeof point.value === "number" && Number.isFinite(point.value)).length;
  const latestFiniteDate = (points: Point[]): string | null => {
    const last = lastNumeric(points);
  ...
  for (const [target, candidates] of Object.entries(aliasCandidates)) {
    ...
    for (const candidate of candidates) {
      ...
      if (!candidateSeries || candidateSeries.length === 0) continue;
      const bestFinite = finiteCount(best);
      const candidateFinite = finiteCount(candidateSeries);
      const bestLatest = latestFiniteDate(best) ?? "";
      const candidateLatest = latestFiniteDate(candidateSeries) ?? "";
      const candidateBetter = candidateFinite > bestFinite
        || (candidateFinite === bestFinite && candidateLatest > bestLatest);
      if (candidateBetter) best = candidateSeries;
    }
    if (best.length > 0) map.set(target, best);
  }
