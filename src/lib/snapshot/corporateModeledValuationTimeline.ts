...
export function buildCorporateModeledValuationTimeline(args: {
  ...
  tpYearLabelMap?: Record<number, string[]>;
  ...
  const markers: CorporateModeledTimelineMarker[] = tps.map((tp) => {
    ...
    const valueHigh = lista2.metrics.DCF_prodStart_present_perShare_TargetCurrency;
    const valueLow = null;
    const nullReasonIfAny = valueHigh === null
    ...
    return {
      tp,
      yearLabelUsed: (() => {
        const labels = args.tpYearLabelMap?.[tp] ?? [];
