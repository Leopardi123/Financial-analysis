...
export async function runCorporateSnapshotPipeline(args: {
  ...
  try {
    ...
    const distinctProductionStartPeriods = Array.from(
      new Set(
        projectsForBuildFunding
          .map((project) => project.productionStartPeriod)
          .filter((tp): tp is number => Number.isInteger(tp)),
      ),
    ).sort((a, b) => a - b);
    ...
    const tpYearLabelMap = projectsForBuildFunding.reduce<Record<number, string[]>>((acc, project) => {
      const tp = project.productionStartPeriod;
      const label = project.periodEndDatesUtc[tp];
      if (!Number.isInteger(tp) || typeof label !== 'string') return acc;
      const existing = acc[tp] ?? [];
      if (!existing.includes(label)) {
        acc[tp] = [...existing, label];
      }
      return acc;
    }, {});
    ...
    snapshot.modeledValuationTimeline = buildCorporateModeledValuationTimeline({
      productionStartPeriods: distinctProductionStartPeriods,
      tpYearLabelMap,
      fcfUSD_total: aggregationEffective.fcffUSD_total,
      masterN: aggregationEffective.corporateMasterN,
      discountRate: input.discountRate,
