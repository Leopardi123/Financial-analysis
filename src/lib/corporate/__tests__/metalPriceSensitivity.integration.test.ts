...
test('survivability scenario family reruns the full Corporate pipeline', async () => {
  ...
  for (const result of results) {
    ...
    const valuationRows = (result.snapshot as any).corporateValuationTimeSeries?.rows ?? [];
    assert.ok(valuationRows.length > 0);
    assert.ok(valuationRows.some((row: any) => typeof row.npvAbsolute === 'number' && typeof row.navAbsolute === 'number'));
  }
