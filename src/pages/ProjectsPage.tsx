...
export default function ProjectsPage() {
  ...
  const pnlDebugger = useMemo(() => {
    ...
    const periodsWithRevenue = Array.from({ length: seriesColumns.length }, (_, t) => t)
      .filter((t) => {
        const gross = pnl.grossRevenue[t];
        return typeof gross === 'number' && Number.isFinite(gross) && gross > 0;
      });
    ...
    const royaltiesPathTraceRevenueYears = {
      totalPeriods: seriesColumns.length,
      revenuePeriodsCount: periodsWithRevenue.length,
      revenuePeriods: periodsWithRevenue,
      chainByRevenuePeriod: periodsWithRevenue.map((periodIndex) => ({
        periodIndex,
        periodLabel: seriesColumns[periodIndex] ?? `t=${periodIndex}`,
        revenueByMetal: Object.fromEntries(
          Object.keys(pnl.revenueByMetal)
            .sort((a, b) => a.localeCompare(b))
            .map((metal) => [metal, pnl.revenueByMetal[metal]?.[periodIndex] ?? null]),
        ),
        grossRevenueResolved: pnl.grossRevenue[periodIndex] ?? null,
        rulesEvaluated: royaltiesRuleTrace.map((rule) => ({
          ...rule,
          royaltyUSD_ifApplied: rule.computable && pnl.grossRevenue[periodIndex] !== null
            ? (pnl.grossRevenue[periodIndex] as number) * ((rule.rateParsed as number) / 100)
            : null,
        })),
        royaltiesResolution: {
          source: pnl.royaltiesSourceUsed,
          royaltyRatePct: pnl.royaltyRatePct[periodIndex] ?? null,
          royaltyUSD: pnl.royalties[periodIndex] ?? null,
          royaltiesDetailFailureReason: pnl.royaltiesDetailFailureReason,
          royaltiesFailureReason: pnl.royaltiesFailureReason,
        },
        downstream: {
          grossProfit: pnl.grossProfit[periodIndex] ?? null,
          ebitda: pnl.ebitda[periodIndex] ?? null,
          ebit: pnl.ebit[periodIndex] ?? null,
          fcff: pnl.fcff[periodIndex] ?? null,
        },
        snapshotComparison: {
          snapshotTotalRevenue: series?.totalRevenue_USD?.[periodIndex] ?? null,
          snapshotRoyaltiesUSD: series?.royaltiesUSD?.[periodIndex] ?? null,
          snapshotEbitUSD: series?.ebitUSD?.[periodIndex] ?? null,
          snapshotFcffUSD: series?.fcffUSD?.[periodIndex] ?? null,
        },
      })),
    };
    ...
    return {
      ...
      royaltiesDebug,
      royaltiesPathTrace,
      royaltiesPathTraceRevenueYears,
    };
    ...
    <div className="projects-shell">
      <main className="projects-card">
        ...
        <section className="projects-series-section">
          ...
          <h3>---- PROJECT GRID P&L DIAGNOSTICS ----</h3>
        ...
        <details>
          ...
          {!pnlDebugger ? (
            ...
            <div className="projects-debugger">
              ...
              <article className="projects-debugger-section">
                <h3>Royalties chain for all revenue years</h3>
                <p>Full chain of royalties assessments for every period where gross revenue is numeric and &gt; 0.</p>
                <pre style={{ whiteSpace: 'pre-wrap' }}>{JSON.stringify(pnlDebugger.royaltiesPathTraceRevenueYears, null, 2)}</pre>
              </article>
