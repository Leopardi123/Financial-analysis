...
function validateProjectJson(rawJson: string): ValidationState {
  ...
  if (version !== 'project_json_v2') {
    if (version === 'project_json_v1') {
      return { ok: false, error: 'This project uses legacy project_json_v1. Open in legacy editor or paste v2 JSON.', warning: null, parsed: null };
    }
    return {
      ok: false,
      error: 'raw.version must be "project_json_v2".',
      warning: null,
      parsed: null,
    };
  }
  ...
  if (Object.prototype.hasOwnProperty.call(time, 'periodEndDatesUtc')) {
    return { ok: false, error: 'time.periodEndDatesUtc is not allowed in project_json_v2. Remove it.', warning: null, parsed: null };
  }
  ...
  if (masterN === null || masterN < 0) {
    return { ok: false, error: 'time.masterN must be an integer >= 0.', warning: null, parsed: null };
  }
  if (tp === null || tp < 0 || tp > masterN) {
    return { ok: false, error: 'time.productionStartPeriod must be an integer in [0, masterN].', warning: null, parsed: null };
  }
