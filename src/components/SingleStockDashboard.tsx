...
{JSON.stringify((() => {
  ...
  const list2Debug = {
    ...
    markerMetricCalculationDebug: (() => {
      const markers = Array.isArray((corporateModeledValuationTimeline as { markers?: Array<Record<string, unknown>> } | null)?.markers)
        ? ((corporateModeledValuationTimeline as { markers?: Array<Record<string, unknown>> }).markers ?? [])
        : [];
      const financing = (corporateSnapshotData?.financing ?? {}) as Record<string, unknown>;
      const sharesPf = typeof financing.shares_post_financing === "number" && Number.isFinite(financing.shares_post_financing) && financing.shares_post_financing > 0
        ? financing.shares_post_financing
      ...
      const debtT0 = typeof financing.debt_t0_post_TargetCurrency === "number" && Number.isFinite(financing.debt_t0_post_TargetCurrency)
        ? financing.debt_t0_post_TargetCurrency
        : null;
      const netCashPerShare = sharesPf !== null && cashT0 !== null && debtT0 !== null ? (cashT0 - debtT0) / sharesPf : null;
      return {
        formulas: {
          DCF_prodStart_exCapex_perShare: "marker.value_high",
          DCF_prodStart_exCapex: "marker.value_high * shares_post_financing",
          NPV_prodStart_perShare: "marker.value_high - netCashPerShare",
          NPV_prodStart: "(marker.value_high - netCashPerShare) * shares_post_financing",
          NAV_prodStart_perShare: "(marker.value_high - netCashPerShare) + netCashPerShare",
          NAV_prodStart: "((marker.value_high - netCashPerShare) + netCashPerShare) * shares_post_financing",
          DCF_prodStart_present_perShare: "marker.value_high * df_now[tIndex]",
          DCF_prodStart_present: "marker.value_high * df_now[tIndex] * shares_post_financing",
        },
        assumptions: {
          markerValueHighRepresents: "DCF_prodStart_exCapex_perShare from modeledValuationTimeline marker payload",
          netCashPerShare,
          shares_post_financing: sharesPf,
        },
        equalityCheckNotes: markers.map((marker) => {
          const high = typeof marker.value_high === "number" && Number.isFinite(marker.value_high) ? marker.value_high : null;
          const npvPerShare = high !== null && netCashPerShare !== null ? high - netCashPerShare : null;
          const navPerShare = npvPerShare !== null && netCashPerShare !== null ? npvPerShare + netCashPerShare : null;
          return {
            tp: typeof marker.tp === "number" ? marker.tp : null,
            markerValueHigh_perShare: high,
            derivedNPV_prodStart_perShare: npvPerShare,
            derivedNAV_prodStart_perShare: navPerShare,
            dcfEqualsNpvPerShare: high !== null && npvPerShare !== null ? Math.abs(high - npvPerShare) <= 1e-9 : null,
            dcfEqualsNavPerShare: high !== null && navPerShare !== null ? Math.abs(high - navPerShare) <= 1e-9 : null,
            whyCanAppearEqual: netCashPerShare === null
              ? "netCashPerShare missing; NPV/NAV prodStart cannot be derived from marker in this view"
              : Math.abs(netCashPerShare) <= 1e-9
                ? "netCashPerShare is ~0, so DCF exCapex, NPV prodStart, NAV prodStart collapse to same value"
                : "non-zero netCashPerShare should create spread between DCF exCapex and NPV/NAV prodStart",
          };
        }),
      };
    })(),
