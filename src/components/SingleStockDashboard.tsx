...
const PRE_REVENUE_CORE_INFO: Record<string, MetricInfoSection[]> = {
  ...
  "D6 Raises in Last 5 Years": buildCoreInfo("Count of periods with positive financing inflow proxy.", "Simpler financing dependence KPI.", "Higher count means repeated external financing needs.", "Frequent raises while runway remains weak.", "Derived from commonStockIssued > 0 over latest 5 periods."),
  "E1 Burn Change YoY (abs)": buildCoreInfo("Year-over-year change in burn rate (currency/month).", "Shows whether burn is improving or worsening.", "Positive means burn worsened vs prior period.", "Repeated positive values before financing events.", "Derived as burn_t - burn_(t-1), burn = abs(FCF_TTM)/12."),
  "E1b Burn Acceleration (Momentum) YoY %": buildCoreInfo("Momentum in burn trend as percent.", "Early warning signal before level changes become obvious.", "Positive = burn worsening faster; negative = slowing deterioration/improvement.", "Repeated positive spikes around funding pressure.", "Based on FCF TTM; requires 3 periods. >500% hidden as unreliable."),
  "E2 Runway vs Risk Thresholds": buildCoreInfo("Runway versus 12m/24m thresholds.", "Makes financing urgency explicit.", "<12m high risk, 12–24m caution, >24m stronger buffer.", "Persistent sub-12m runway.", "Derived from runway months."),
  "E3 Dilution vs Runway (Scatter)": buildCoreInfo("Scatter of dilution % (y) vs runway months (x).", "Tests whether dilution buys enough runway.", "Favorable region: lower dilution with longer runway.", "Toxic quadrant: high dilution + low runway.", "Point shown only when both runway and dilution are available."),
...
export default function SingleStockDashboard() {
  ...
  const raisesInLast5Years = commonStockIssuedSeries.slice(-5).filter((value) => typeof value === "number" && value > 0).length;

  const burnChangeYoYData = buildDerivedSeries(["Date", "Burn Change YoY (abs)"], (index) => {
    if (index === 0) return [null];
    const burn = burnRateTtmData?.[index + 1]?.[1] as number | null | undefined;
    const previous = burnRateTtmData?.[index]?.[1] as number | null | undefined;
    if (typeof burn !== "number" || typeof previous !== "number") return [null];
    return [burn - previous];
  }, 15);

  const burnAccelerationMomentumPctData = buildDerivedSeries(["Date", "Burn Acceleration (Momentum) YoY %"], (index) => {
    if (index < 2) return [null];
    const burn = burnRateTtmData?.[index + 1]?.[1] as number | null | undefined;
    const prev = burnRateTtmData?.[index]?.[1] as number | null | undefined;
    const prevPrev = burnRateTtmData?.[index - 1]?.[1] as number | null | undefined;
    if (typeof burn !== "number" || typeof prev !== "number" || typeof prevPrev !== "number" || prev <= 0) return [null];
    const accPct = 100 * (((burn - prev) - (prev - prevPrev)) / prev);
    if (Math.abs(accPct) > 500) return [null];
    return [accPct];
  }, 15);
  const runwayRiskBandsData = buildDerivedSeries(["Date", "Runway", "12M", "24M"], (index) => {
    const runway = runwayMonthsData?.[index + 1]?.[1] as number | null | undefined;
    return [typeof runway === "number" ? runway : null, 12, 24];
  }, 15);
  const dilutionVsRunwayData = buildDerivedSeries(["Runway Months", "Dilution Rate %"], (index) => {
    const dilution = dilutionRateData?.[index + 1]?.[1] as number | null | undefined;
    const runway = runwayMonthsData?.[index + 1]?.[1] as number | null | undefined;
    return typeof runway === "number" && typeof dilution === "number" ? [runway, dilution] : [null, null];
  }, 15);
  ...
  const survivalComponentData = buildDerivedSeries(["Date", "Survival", "Dilution", "Discipline"], (index) => {
    const runway = runwayMonthsData?.[index + 1]?.[1] as number | null | undefined;
    const dilution = dilutionRateData?.[index + 1]?.[1] as number | null | undefined;
    const overheadPct = overheadRatioData?.[index + 1]?.[1] as number | null | undefined;
    if (typeof runway !== "number") return [null, null, null];
    const survival = Math.max(0, Math.min(4, runway / 6));
    const dilutionComponent = Math.max(0, Math.min(3, 3 - ((dilution ?? 0) / 15)));
    const discipline = Math.max(0, Math.min(3, 3 - ((overheadPct ?? 0) / 100)));
  ...
  const latestValueFromSeries = (series: (string | number | Date | null)[][] | null, col = 1): number | null => {
    if (!series || series.length < 2) return null;
    const row = series[series.length - 1];
    const value = row?.[col];
    return typeof value === "number" && Number.isFinite(value) ? value : null;
  };
  const latestRunway = latestValueFromSeries(runwayMonthsData);
  const latestCash = latestValueFromSeries(buildSeriesData(buildSeries(data, [{ label: "Cash Balance", statement: "balance", field: "cashAndCashEquivalents" }]), 15));
    ...
    <div className="single-stock-dashboard">
      ...
      {analysisMode === "prerevenue" && primaryView === "reported" && (
        <>
          ...
          <div className="breadcontainerquadrupelcolumn">
            <div className="chart-card"><p className="bread">Runway Months</p><p className="subrub">{latestRunway === null ? "—" : latestRunway.toLocaleString("en-US", { maximumFractionDigits: 1 })}</p><p className="bread">{runwayRiskLabel}</p></div>
            <div className="chart-card"><p className="bread">Cash Balance</p><p className="subrub">{latestCash === null ? "—" : latestCash.toLocaleString("en-US", { maximumFractionDigits: 0 })}</p></div>
            <div className="chart-card"><p className="bread">Burn Rate TTM</p><p className="subrub">{latestBurn === null ? "—" : latestBurn.toLocaleString("en-US", { maximumFractionDigits: 1 })}</p></div>
            <div className="chart-card"><p className="bread">Dilution YoY</p><p className="subrub">{latestDilution === null ? "—" : latestDilution.toLocaleString("en-US", { maximumFractionDigits: 1 })}%</p></div>
            <div className="chart-card"><p className="bread">Governance Leak Index</p><p className="subrub">{latestGovernanceLeak === null ? "—" : latestGovernanceLeak.toLocaleString("en-US", { maximumFractionDigits: 3 })}</p></div>
            <div className="chart-card"><p className="bread">Survival Score</p><p className="subrub">{latestSurvivalScore === null ? "—" : latestSurvivalScore.toLocaleString("en-US", { maximumFractionDigits: 2 })}</p></div>
          </div>
          ...
          <div className="chartcontainerdoublecolumn">
            ...
            <div className="chart-card"><div className="producer-core-title-row"><h3 className="subrub small">D6 Raises in Last 5 Years</h3><InfoPopover id="d6-raises-kpi" openId={openInfoId} onToggle={(id) => setOpenInfoId((prev) => (prev === id ? null : id))} onClose={() => setOpenInfoId(null)} title="D6 Raises in Last 5 Years" sections={withUnitMetadata(PRE_REVENUE_CORE_INFO["D6 Raises in Last 5 Years"], "count", "unknown", mixedCurrencyNote)} /></div><p className="subrub">{raisesInLast5Years}</p></div>
          </div>

          <div className="breadcontainersinglecolumn"><h2 className="subrub small">E) Risk Signals & Scoreboard</h2></div>
          <div className="chartcontainerdoublecolumn">
            <ReportedChart fiscalYearEndMonth={fiscalYearEndMonth} chartType="ColumnChart" title="E1 Burn Change YoY (abs)" id="E1 Burn Change YoY (abs)" infoSections={PRE_REVENUE_CORE_INFO["E1 Burn Change YoY (abs)"]} openInfoId={openInfoId} onToggleInfo={(id) => setOpenInfoId((prev) => (prev === id ? null : id))} onCloseInfo={() => setOpenInfoId(null)} data={burnChangeYoYData} />
            <ReportedChart fiscalYearEndMonth={fiscalYearEndMonth} chartType="ColumnChart" title="E1b Burn Acceleration (Momentum) YoY %" id="E1b Burn Acceleration (Momentum) YoY %" infoSections={PRE_REVENUE_CORE_INFO["E1b Burn Acceleration (Momentum) YoY %"]} openInfoId={openInfoId} onToggleInfo={(id) => setOpenInfoId((prev) => (prev === id ? null : id))} onCloseInfo={() => setOpenInfoId(null)} data={burnAccelerationMomentumPctData} options={{ vAxis: { format: "#,##0.##'%'" } }} />
