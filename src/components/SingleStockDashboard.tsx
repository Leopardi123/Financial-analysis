...
Capital Available: ${availableLabel}`,
  ...
  const corporateViewMetrics = useMemo(() => {
    ...
    const dcfSeriesRaw = Array.isArray(prodStartDebug.dcfProdStartSeries)
      ? prodStartDebug.dcfProdStartSeries as Array<Record<string, unknown>>
      : [];
    const prodSeries = dcfSeriesRaw
      .map((point, idx) => {
        const year = typeof point.year === "number" && Number.isFinite(point.year) ? point.year : null;
        const dcfProdStartPresentTarget = typeof point.valueTargetCurrency === "number" && Number.isFinite(point.valueTargetCurrency)
          ? point.valueTargetCurrency
          : null;
        const npvProdStartTarget = typeof point.npvProdStartTargetCurrency === "number" && Number.isFinite(point.npvProdStartTargetCurrency)
          ? point.npvProdStartTargetCurrency
          : null;
        const navProdStartTarget = typeof point.navProdStartTargetCurrency === "number" && Number.isFinite(point.navProdStartTargetCurrency)
          ? point.navProdStartTargetCurrency
          : null;
        return { tp: idx, year, dcfProdStartPresentTarget, npvProdStartTarget, navProdStartTarget };
      })
