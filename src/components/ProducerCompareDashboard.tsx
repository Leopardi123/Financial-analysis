...
function formatReportedMetricPeriod(metric: ReportedMetric): string {
  const period = metric.period;
  if (period.kind === 'year') return String(period.year);
  if (period.kind === 'year_range_average') return `rapporterat snitt ${period.startYear}–${period.endYear}`;
  if (period.kind === 'year_range_total') return `rapporterad summa ${period.startYear}–${period.endYear}`;
...
function reportedMetricToOzRange(metric: ReportedMetric | null, expectedMetric: 'production' | 'aueq'): NumericRange | null {
  if (!metric || metric.metric !== expectedMetric) return null;
...
function reportedProductionToAuOzRange(metric: ReportedMetric | null): NumericRange | null {
  return reportedMetricToOzRange(metric, 'production');
}

function reportedAuEqToOzRange(metric: ReportedMetric | null): NumericRange | null {
  return reportedMetricToOzRange(metric, 'aueq');
}
              ...
              <table className="producer-compare__table">
                ...
                <tbody>
                  {data.table.rows.map((row) => {
                    ...
                    const reportedAuEqRange = reportedAuEqToOzRange(row.reportedAuEq);
                    const reportedProxyQuantityRange = reportedAuEqRange ?? reportedAuRange;
                    const reportedProxyQuantityLabel = reportedAuEqRange ? 'rapporterad AuEq' : 'rapporterad Au';
                    const selectedAuPrice = deck?.pricesByMetal.Au;
                    const reportedRevenueProxy = reportedProxyQuantityRange && selectedAuPrice?.valueUSD !== null && selectedAuPrice?.valueUSD !== undefined && selectedAuPrice.unit === 'USD_per_toz'
                      ? multiplyPositiveRanges(reportedProxyQuantityRange, { low: selectedAuPrice.valueUSD, high: selectedAuPrice.valueUSD })
                      : null;
                    ...
                    const reportedCashCostSpend = reportedProxyQuantityRange && reportedCashCostPerOz
                      ? multiplyPositiveRanges(reportedProxyQuantityRange, reportedCashCostPerOz)
                      : null;
                    const reportedAiscSpend = reportedProxyQuantityRange && reportedAiscPerOz
                      ? multiplyPositiveRanges(reportedProxyQuantityRange, reportedAiscPerOz)
                      : null;
                    ...
                    const mcapAuRange = row.marketCapUSD !== null
                      ...
                      : null;
                    const mcapAuEqDenominator = canonicalAuEqRange ?? reportedAuEqRange;
                    const mcapAuEqRange = row.marketCapUSD !== null && mcapAuEqDenominator
                      ? valuePerPositiveRange(row.marketCapUSD, mcapAuEqDenominator)
                      : null;
                    ...
                    const preferSelectedDeckRevenueProxy = data.table.priceMode !== 'REPORTED' && reportedRevenueProxy !== null;
                      ...
                      <tr key={row.companyId}>
                        ...
                        <td className="producer-compare__evidence-cell">
                          {row.revenueUSD !== null ? formatUsd(row.revenueUSD) : canonicalRevenueRange ? (
                            ...
                            <div className="producer-compare__evidence-list">
                              ...
                              {row.reportedRevenue && (
                                <small>Källans egen revenue: {formatClaim(row.reportedRevenue)} ({formatReportedMetricPeriod(row.reportedRevenue)}; ej reprissatt)</small>
                              )}
                            </div>
                          ) : row.reportedRevenue ? (
                            <div className="producer-compare__evidence-item">
                              <strong>{formatClaim(row.reportedRevenue)}</strong>
                              <small>Rapporterad revenue · {formatReportedMetricPeriod(row.reportedRevenue)} · ej reprissatt till annat deck</small>
                            </div>
                          ) : reportedRevenueProxy ? (
                            <IntervalValue range={reportedRevenueProxy} label={`${reportedProxyQuantityLabel} × valt ${data.table.priceMode}-Au-pris · proxy, ej canonical financial revenue`} />
                          ) : 'Ej beräkningsbart'}
                        </td>
                        <td className="producer-compare__evidence-cell">
                        ...
                        <td className="producer-compare__evidence-cell">
                          {row.ebitdaUSD !== null ? formatUsd(row.ebitdaUSD) : canonicalEbitdaRange ? (
                            ...
                            <IntervalValue range={operatingCashMarginProxy} label={`Cash-margin proxy: selected-price revenue − reported cash cost × ${reportedProxyQuantityLabel}. Före royalty/G&A m.m.; INTE EBITDA.`} />
                          ) : 'Ej beräkningsbart'}
                        </td>
                        <td className="producer-compare__evidence-cell">
                          {row.fcffBeforeGrowthUSD !== null ? formatUsd(row.fcffBeforeGrowthUSD) : canonicalFcffBeforeRange ? (
                            ...
                            <IntervalValue range={aiscMarginProxy} label={`AISC-margin proxy: selected-price revenue − reported AISC × ${reportedProxyQuantityLabel}. Före cash tax/WC och med AuEq/deck-risk; INTE FCFF.`} />
                          ) : 'Ej beräkningsbart'}
                        </td>
                        <td className="producer-compare__evidence-cell">
                        ...
                        <td className="producer-compare__evidence-cell">
                          {row.marketCapPerAuEqOzUSD !== null ? `${formatUsd(row.marketCapPerAuEqOzUSD)}/oz` : mcapAuEqRange ? (
                            <IntervalValue
                              ...
                              label={canonicalAuEqRange ? 'MCap / kanoniskt fysisk AuEq-intervall' : 'MCap / rapporterad AuEq · proxy, ej kanonisk fysisk AuEq-multipel'}
                            />
