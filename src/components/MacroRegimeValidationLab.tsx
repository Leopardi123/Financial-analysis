...
export default function MacroRegimeValidationLab() {
  ...
  const [coverageFillRunning, setCoverageFillRunning] = useState(false);
  const [coverageFillStatus, setCoverageFillStatus] = useState<string | null>(null);

  useEffect(() => {
    if (typeof window === "undefined") return;
    const saved = window.localStorage.getItem("macroLab.debugAdminSecret") ?? "";
    if (saved) setAdminSecretInput(saved);
  }, []);

  useEffect(() => {
    if (typeof window === "undefined") return;
    window.localStorage.setItem("macroLab.debugAdminSecret", adminSecretInput);
  }, [adminSecretInput]);

  async function runCoverageBackfill() {
    setCoverageFillRunning(true);
    setCoverageFillStatus(null);
    try {
      const ingestResponse = await fetch(`/api/admin/macro/ingest?mode=backfill&region=${region}`, {
        method: "POST",
        headers: adminSecretInput.trim() ? { "x-admin-secret": adminSecretInput.trim() } : undefined,
      });
      const ingestPayload = await ingestResponse.json();
      if (!ingestResponse.ok) {
        throw new Error(String((ingestPayload as { error?: string })?.error ?? "Ingest backfill failed"));
      }

      const engineResponse = await fetch(`/api/admin/macro/run-engine?region=${region}`, {
        method: "POST",
        headers: adminSecretInput.trim() ? { "x-admin-secret": adminSecretInput.trim() } : undefined,
      });
      const enginePayload = await engineResponse.json();
      if (!engineResponse.ok) {
        throw new Error(String((enginePayload as { error?: string })?.error ?? "Engine run failed"));
      }

      setCoverageFillStatus("25y backfill + engine run klart. Laddar om data…");
      ...
      setCoverageFillStatus(error instanceof Error ? error.message : "Backfill misslyckades");
    } finally {
      setCoverageFillRunning(false);
    }
  }
  ...
  return (
    <div className="macro-lab">
      ...
      <div className="macro-lab-admin-row">
        <label>Admin secret
          <input type="password" value={adminSecretInput} onChange={(e) => setAdminSecretInput(e.target.value)} placeholder="för 25y backfill" />
        </label>
        <button type="button" onClick={() => void runCoverageBackfill()} disabled={coverageFillRunning}>
          {coverageFillRunning ? `Fyller 25y ${region}...` : `Fyll 25y data (${region})`}
        </button>
        {coverageFillStatus && <span className="macro-lab-admin-status">{coverageFillStatus}</span>}
      </div>
