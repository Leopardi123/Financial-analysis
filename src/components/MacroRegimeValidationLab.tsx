...
type MacroLabPayload = { macroHistory?: { points?: MacroHistoryPoint[] }; inflationAnalysis?: { points?: InflationPoint[] }; globalMacro?: { macroExplanation?: MacroExplanation; macroRegimeProbability?: { primaryRegime: string | null; primaryWeight: number | null; decisiveness: number | null; transitionLike: boolean; distribution: Array<{ regime: string; weight: number }>; narrative?: { short?: string; medium?: string; long?: string }; structuralAdjustment?: { summary?: string; multiplier?: number; penalty?: number }; regimeMomentum?: { direction?: string; momentumScore?: number; primaryRegimeChange?: string; driftTowardRegime?: string | null; changeDrivers?: string[]; narrative?: string }; overlayInfluence?: { primarySignal?: string; candidateSignals?: Array<{ regime: string; signal: string }>; summary?: string }; supportingBlocks?: string[]; supportingOverlays?: string[]; modulatingOverlays?: string[]; contradictingOverlays?: string[] } | null } };
          ...
          {labRegimeProbability ? (
            <div className="macro-lab-note" style={{ marginBottom: 8 }}>
              ...
              <details style={{ marginTop: 6 }}>
                ...
                <div style={{ fontSize: 12 }}>Modulating overlays: {(labRegimeProbability.modulatingOverlays ?? []).join(", ") || "—"}</div>
                <div style={{ fontSize: 12 }}>Contradicting overlays: {(labRegimeProbability.contradictingOverlays ?? []).join(", ") || "—"}</div>
                <div style={{ fontSize: 12 }}>Structural adjustment: {labRegimeProbability.structuralAdjustment?.summary ?? "none"} · multiplier {typeof labRegimeProbability.structuralAdjustment?.multiplier === "number" ? labRegimeProbability.structuralAdjustment.multiplier.toFixed(2) : "—"} · penalty {typeof labRegimeProbability.structuralAdjustment?.penalty === "number" ? labRegimeProbability.structuralAdjustment.penalty.toFixed(2) : "—"}</div>
                <div style={{ fontSize: 12 }}>Momentum: {labRegimeProbability.regimeMomentum?.direction ?? "stable"} · score {typeof labRegimeProbability.regimeMomentum?.momentumScore === "number" ? labRegimeProbability.regimeMomentum.momentumScore.toFixed(1) : "—"} · primary change {labRegimeProbability.regimeMomentum?.primaryRegimeChange ?? "—"} {labRegimeProbability.regimeMomentum?.driftTowardRegime ? `→ ${labRegimeProbability.regimeMomentum.driftTowardRegime}` : ""}</div>
                <div style={{ fontSize: 12 }}>Change drivers: {(labRegimeProbability.regimeMomentum?.changeDrivers ?? []).join(", ") || "—"}</div>
                <div style={{ fontSize: 12 }}>Overlay influence: {labRegimeProbability.overlayInfluence?.primarySignal ?? "—"} · {(labRegimeProbability.overlayInfluence?.candidateSignals ?? []).map((row) => `${row.regime}:${row.signal}`).join(", ") || "—"}</div>
                <div style={{ fontSize: 12 }}>Overlay summary: {labRegimeProbability.overlayInfluence?.summary ?? "—"}</div>
