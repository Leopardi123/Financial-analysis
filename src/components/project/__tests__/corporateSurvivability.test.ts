...
const snapshot = (fcff: number[], years = fcff.map((_, index) => 2026 + index), productionStartYear = 2026, capex = fcff.map(() => 0)) => ({
  ...
  corporateValuationTimeSeries: { valuationYear: 2026, rows: years.map((year, index) => ({ year, npvAbsolute: 50 - index * 5, navAbsolute: 45 - index * 7 })), projectMarkers: [{ productionStartYear, productionStartPeriod: years.indexOf(productionStartYear) }] },
...
test('analysis excludes historical and construction years and measures only operating funding', () => {
  ...
  assert.deepEqual(model.valuationRows.map((row) => row.year), [2028, 2029]); assert.equal(model.targetCurrency, 'SEK');
});
...
test('survivability page contract explains operating bars and separates production-start build CAPEX', async () => {
  ...
  for (const token of ['Closing cash','Minimum reserve','Negativ FCFF','negative-fcff','täcks av cash som byggts upp tidigare','Likviditet','Värdekurvor','Stress-NPV, kvarvarande värde','Canonical stress-NAV','Värde, inte likviditet','canonical net-cash bridge','Unfunded gap','Operating debt raised','inte skuldstock, ränta eller amortering','Varför syns stapeln','Övriga visade år behöver ingen ny driftfinansiering','Initial/build CAPEX','Construction funding need','role="dialog"','aria-pressed','Produktionsstopp','Kräver högre upplösning i produktionsmodellen.','Corporate survivability analysis']) assert.match(source, new RegExp(token));
