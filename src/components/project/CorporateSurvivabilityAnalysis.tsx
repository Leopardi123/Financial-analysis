...
function SurvivabilityChart({ model, selectedYear }: { model: SurvivabilityModel; selectedYear: number | null }) {
  ...
  return <div className="survivability-chart-scroll" role="group" aria-label={`Corporate cash runway för ${model.label}`}>
    ...
    <svg className="survivability-chart survivability-funding-chart" viewBox={`0 0 ${width} ${height}`} aria-label="Debt, equity och unfunded gap per kalenderår">
    ...
    <div className="survivability-legend"><span className="cash">Closing cash</span><span className="reserve">Minimum reserve</span><span className="debt">Debt</span><span className="equity">Equity</span><span className="gap">Unfunded gap</span></div>
  </div>;
}
...
export function CorporateSurvivabilityAnalysis(props: { models: Map<SurvivabilityScenarioId, SurvivabilityModel>; financingMode: SurvivabilityFinancingMode; onFinancingModeChange: (mode: SurvivabilityFinancingMode) => void; loading: boolean; error?: string | null; performanceText?: string | null }) {
  ...
  return <section className="corporate-survivability" aria-label="Corporate survivability analysis">
    ...
    {drawerOpen && criticalRow && model && <aside className="survivability-drawer" role="dialog" aria-modal="false" aria-labelledby="critical-year-title"><header><div><p className="eyebrow">CRITICAL YEAR</p><h4 id="critical-year-title">{model.label} · {criticalRow.year}</h4></div><button type="button" aria-label="Stäng critical year" onClick={() => setDrawerOpen(false)}>×</button></header><div className="survivability-drawer-grid">{[['Opening cash',criticalRow.openingCash],['Operating cash',criticalRow.operatingCashGenerated],['Construction CAPEX',criticalRow.constructionCapex],['Internal cash',criticalRow.internalCashUsed],['Debt',criticalRow.debtAdded],['Equity',criticalRow.equityRaised],['Unfunded gap',criticalRow.unfundedGap],['Closing cash',criticalRow.closingCash],['FCFF',criticalRow.fcff]].map(([label,value])=><div key={String(label)}><span>{label}</span><strong>{format(String(label), value as number|null)}</strong></div>)}</div><h5>Projektbidrag</h5><div className="survivability-projects">{Object.keys({ ...criticalRow.operatingCashGeneratedByProject, ...criticalRow.constructionCapexByProject, ...criticalRow.debtAddedByProject, ...criticalRow.equityRaisedByProject }).map((projectId)=><div key={projectId}><strong>{projectId}</strong><span>FCFF/operating: {format('op',criticalRow.operatingCashGeneratedByProject[projectId] ?? null)}</span><span>Construction: {format('capex',criticalRow.constructionCapexByProject[projectId] ?? null)}</span><span>Debt: {format('debt',criticalRow.debtAddedByProject[projectId] ?? 0)}</span><span>Equity: {format('equity',criticalRow.equityRaisedByProject[projectId] ?? 0)}</span><span>Shares: {format('shares',criticalRow.newSharesByProject[projectId] ?? 0)}</span></div>)}</div>{model.diagnostics.length > 0 && <details><summary>Diagnostik</summary><ul>{model.diagnostics.map((item,index)=><li key={`${index}-${item}`}>{item}</li>)}</ul></details>}</aside>}
  </section>;
}
