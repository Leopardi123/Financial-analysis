...
export function buildCorporateSurvivabilityModel(args: {
  ...
  const valuationRows = analysisStartYear === null ? [] : (timeline?.rows ?? [])
    .filter((row): row is { year: number; npvAbsolute?: number | null; navAbsolute?: number | null } => finite(row.year) && row.year >= analysisStartYear)
    .map((row) => ({ year: row.year, npvAbsolute: finite(row.npvAbsolute) ? row.npvAbsolute : null, navAbsolute: finite(row.navAbsolute) ? row.navAbsolute : null }));
