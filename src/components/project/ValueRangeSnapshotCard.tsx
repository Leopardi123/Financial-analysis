        ...
        <div className="project-value-snapshot-wrap">
          <svg viewBox={`0 0 ${VIEWBOX_WIDTH} ${VIEWBOX_HEIGHT}`} role="img" aria-label="Snapshot med NPV- och TP-intervall per aktie" style={{ width: "100%", height: "100%" }}>
            ...
            {hasNpv && hasTp && (
              <polygon
                points={(() => {
                  const valid = tpSeriesX.filter((entry) => entry.lowY !== null && entry.highY !== null);
                  if (valid.length === 0) return "";
                  const highPath = valid.map((entry) => `${entry.x},${entry.highY}`).join(" ");
                  const lowPath = [...valid].reverse().map((entry) => `${entry.x},${entry.lowY}`).join(" ");
                  return `${X_LEFT},${points.npvHighY!} ${highPath} ${lowPath} ${X_LEFT},${points.npvLowY!}`;
                })()}
