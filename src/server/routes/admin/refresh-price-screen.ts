...
async function classifyHeavyCandidates(symbols: string[]): Promise<Set<string>> {
  const normalized = symbols.map((item) => item.trim().toUpperCase()).filter(Boolean);
  ...
  const historyRows = await withTimeout(query(
    `SELECT symbol, COUNT(*) AS c
     ...
     GROUP BY symbol`,
    normalized,
  ), 2000, "db_classify_history_bulk") as Array<{ symbol?: string; c?: number | string }>;
  const snapshotRows = await withTimeout(query(
    `SELECT symbol
     FROM ${tables.priceScreenSnapshot}
     WHERE symbol IN (${placeholders})`,
    normalized,
  ), 2000, "db_classify_snapshot_bulk") as Array<{ symbol?: string }>;

  const historyBySymbol = new Map<string, number>();
  for (const row of historyRows) {
    const symbol = String(row.symbol ?? "").trim().toUpperCase();
    if (!symbol) continue;
    historyBySymbol.set(symbol, Number(row.c ?? 0));
  }
  ...
  for (const symbol of normalized) {
    const historyCount = Number(historyBySymbol.get(symbol) ?? 0);
    const hasSnapshot = snapshotSet.has(symbol);
    ...
    while (pending.length > 0) {
      ...
      for (const symbol of chunk) {
        ...
        try {
          ...
          const asIngest = error as { message?: string; stage?: string; classification?: string; context?: Record<string, unknown> };
          const message = typeof asIngest.message === "string" ? asIngest.message : String(error);
          const retryMap = persistedPayload.retryMap ?? {};
          const attempt = Number(retryMap[symbol] ?? 0) + 1;
          const classification = typeof asIngest.classification === "string"
            ? asIngest.classification
            : (message.toLowerCase().includes("timeout") ? "timeout_during_symbol_fetch" : "unknown");
          chunkFailures.push({
            symbol,
            error: message,
            stage: typeof asIngest.stage === "string" ? asIngest.stage : detectStage(message),
            classification,
