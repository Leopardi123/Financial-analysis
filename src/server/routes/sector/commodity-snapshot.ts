...
export default async function handler(req: any, res: any) {
  try {
    ...
    for (const key of indicatorKeys) {
      ...
      const requiresDemandFields = key === "pmi_china" || key === "pmi_us";
      const isUsableCandidate = (entry: IndicatorRow) => {
        const hasLatest = typeof entry.value_latest === "number" && Number.isFinite(entry.value_latest);
        const hasChange3m = typeof entry.change_3m === "number" && Number.isFinite(entry.change_3m);
        if (requiresDemandFields) return hasLatest && hasChange3m;
        return hasLatest || hasChange3m || (typeof entry.percentile_10y === "number" && Number.isFinite(entry.percentile_10y));
      };
      const preferredUsable = candidates.find((entry) => entry.region === preferredRegion && isUsableCandidate(entry));
      const anyUsable = candidates.find((entry) => isUsableCandidate(entry));
      const preferredAny = candidates.find((entry) => entry.region === preferredRegion);
      const row = preferredUsable ?? anyUsable ?? preferredAny ?? candidates[0] ?? null;
      if (!row) {
      ...
      indicatorSelectionDebug.push({
        ...
        note: (() => {
          if (row.region === preferredRegion && isUsableCandidate(row)) return `Selected preferred region=${preferredRegion} with usable payload.`;
          if (row.region !== preferredRegion && isUsableCandidate(row)) return `Preferred region=${preferredRegion} missing/invalid; used usable fallback region=${row.region}.`;
          return `Selected fallback row but payload is incomplete (value_latest/change_3m may be null).`;
        })(),
      });
    }
