...
export default async function handler(req: any, res: any): Promise<void> {
  ...
  try {
    ...
    const usesNextProjectIrr = preparedProjects.length > 1
      || preparedProjects.some((project) => (project.yearsByPeriod[project.productionStartPeriod] ?? Infinity) <= valuationYear);
