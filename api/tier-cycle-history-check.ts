import type { PriceKey } from '../src/lib/prices/keys.ts';
import { readHistoryRowsInRange } from '../src/lib/prices/db/readHistory.ts';
import { refreshHistoryRangeToMonthlyBlobs } from '../src/lib/prices/refreshHistory.ts';
import { computeTier1CycleMultiplier, toMonthlyLast } from '../src/lib/tier1/cycle.ts';
...
export default async function handler(req: any, res: any): Promise<void> {
  ...
  try {
    let before = await readHistoryRowsInRange({ priceKey: priceKey as PriceKey, from, to });
