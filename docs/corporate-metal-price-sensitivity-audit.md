...
The same fan-out previously repeated live metal-price resolution for every scenario and every project price key. For a two-project Au/Ag/Cu company this could produce at least five primary metal lookups per scenario (and additional Cu fallback lookups when the primary feed was unavailable), despite all seven scenarios sharing the same spot anchor. The base snapshot now publishes its resolver-proven spot deck by project and price key. The sensitivity runner pins that immutable deck into each scenario request; the server consumes it through the normal fixed price-series resolver and applies the multiplier only afterward, before the project engine. This reduces sensitivity-related metal-provider calls to zero while retaining exact scenario economics and per-project metal provenance. Integration tests assert complete pinned/unpinned economic parity and assert that pinned runs contain no `Spot resolver failed` provider diagnostic.

Seven Corporate snapshot HTTP calls and seven full economic calculations remain intentional: each scenario must be isolated and recompute financing, project economics, aggregation, valuation and quality. In symbol mode those calls still perform server-side stored-project reads. They do not consume FMP quota, and replacing them with a batch endpoint would be a separate latency optimization rather than a market-data quota fix. Base financial-statement/profile data is not refetched by sensitivity requests. Net external market-data effect after both corrections, when the base snapshot is computable: **0 additional FX calls and 0 additional metal-price calls**.
